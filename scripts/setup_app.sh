...
for module in "${APP_MODULES[@]}"; do
  ...
  echo "⚠️  $ORG_NAME/$module already exists. Will only change remote and push."
else
  echo "🌐 Creating repo $ORG_NAME/$module on GitHub..."
  gh repo create "$ORG_NAME/$module" --private --confirm
fi

echo "🔁 Setting correct remote for $module..."
cd "$module" || exit 1
...
# === 📋 Mostrar resumen final de remotes ===
echo ""
echo "📦 Resumen de repos clonados:"
printf "   %-15s → %s\n" "Carpeta" "Remote"
echo "────────────────────────────────────────────────────────────"

for module in "${APP_MODULES[@]}"; do
  REMOTE="git@github.com:$ORG_NAME/$module.git"
  printf "🟢 %-15s → %s\n" "$module" "$REMOTE"
done

for shared in "${SHARED_MODULES[@]}"; do
  REMOTE="git@github.com:$CHICBASE_ORG/$shared.git"
  printf "🔵 %-15s → %s\n" "$shared" "$REMOTE"
done

echo ""
echo "🚀 Ábrelo en VS Code con:"
echo "   code app.code-workspace"
echo ""
